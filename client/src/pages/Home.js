...
function WelcomeText() {
  ...
  return (
    <div className="text-center md:text-left flex flex-col ">
      <img src={black_star} className=" w-1/8 md:hidden item-center m-auto spin"></img>
      <h1 className="font-bold font-inter text-black text-3xl md:text-5xl lg:text-6xl mt-5 pt-1 md:py-3">hey {userData.display_name}.</h1>
      <h2 className="font-inter text-black text-3xl md:text-5xl lg:text-6xl font-light md:font-normal pt-1 md:py-3">what stats would you like to look at?</h2>
    </div>
...
function NavButton({ linkto, buttontext }) {
  return (
    <div>
        <NavLink to={linkto}>
          <button className="transition-colors duration-300 bg-green md:px-10 px-10 py-5 md:py-0 md:h-16 lg:h-20 rounded-full text-black font-bold text-2xl md:text-lg lg:text-xl xl:text-2xl w-fit hover:bg-black hover:text-white focus:animate-ping ">
              {buttontext}
          </button>
        </ NavLink>
      </div>
